      ...
      <div class="crd">
        ...
        <ul class="list-group list-group-flush">
          ...
          <li class="list-group-item">
            ...
            <p>
              Answer: 1. SQL is the relational database,where NOSQL is
              distributed database. 2. SQL is table based,NOSQL is document
              based. 3. SQL update and insert synchronusly,but NOSQL update and
              insert asynchronusly.
            </p>
          </li>
          <li class="list-group-item">
            <h3>What is the purpose of jwt and how does it works?</h3>
            <p>Answer: The purpose of jwt is to secure API.If anyone wish to know my api data he can easily get it.But we use jwt token it ensure the secured API. When a user log in to the website,a token generates.Server send this token to client.Client store the token to local storage or browser cookies.Next time when the client makes a request, a copy of token is send to the server for authorization.Server verifies the JWT signature before giving the authorization.Server responds to the client request.</p>
          </li>
        </ul>
      </div>
    </div>
